...
%% can take place.inputClausesmd :- 
 Finally, we have a list of possible moderators and
...
writeClauses:- 
    inputClausesed,
    inputClausesem,
    eachDayAtMostMaxEventsPerDay,
    eachEventExactly1Day,
    eachEventExactly1Mod,
    eachModeratorAtMostMaxDaysPerModerator,
    unirmd,

    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

inputClausesed :- event(E), eventDays(E, D1), findall(ed(E,D), member(D, D1), Lits), writeClause(Lits), fail.
inputClausesed.

inputClausesem :- event(E), eventModerators(E, M1), findall(em(E, M), member(M, M1), Lits), writeClause(Lits), fail.
inputClausesem.

unirmd :- event(E), day(D), moderator(M), eventDays(E, D1), eventModerators(E, M1), member(D, D1), member(M, M1), append([], [-ed(E, D), -em(E, M), md(M, D)], Lits), writeClause(Lits), fail.
unirmd.

eachDayAtMostMaxEventsPerDay :- day(D), findall(ed(E, D), event(E), Lits), maxEventsPerDay(N), atMost(N, Lits), fail.
eachDayAtMostMaxEventsPerDay.

eachEventExactly1Day :- event(E), findall(ed(E, D), day(D), Lits), exactly(1, Lits), fail.
eachEventExactly1Day.

eachEventExactly1Mod :- event(E), findall(em(E, M), moderator(M), Lits), exactly(1, Lits), fail.
eachEventExactly1Mod.

eachModeratorAtMostMaxDaysPerModerator :- moderator(M), findall(md(M, D), day(D), Lits), maxDaysPerModerator(N), atMost(N, Lits), fail.
eachModeratorAtMostMaxDaysPerModerator.
