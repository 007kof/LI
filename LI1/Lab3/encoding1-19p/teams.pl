...
incompatibleWorkers(W1,W2) :- workerScore(W1, S1), workerScore(W2, S2), maxScore(T), S is S1 + S2, S > T, W1 \= W2. %% SHOULD BE COMPLETED!!!!
...
eachWorkerExactly1 :- worker(W), findall(wt(W, T), team(T), Lits), exactly(1, Lits), fail.
eachWorkerExactly1.

eachPairWorkersAtMostMaxScore :- worker(W1), worker(W2), incompatibleWorkers(W1, W2), team(T), append([wt(W1, T)], [wt(W2, T)], Lits), atMost(1, Lits), fail.
eachPairWorkersAtMostMaxScore.
