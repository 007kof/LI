...
courseProfessors(C,LP):- course(_,C,_,_,LP).zz
...
satVariable( cy(C,Y) ) :- course(C), year(Y).
...
writeClauses(MaxNumProf):-
    %eachDayMaxOneCourse,
    eachDayWeekMaxNumHoursPerWeek,
    %eachCourseExactlyItsYear,
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachDayMaxOneCourse :- day(D), findall(cd(C,D), course(C), Lits), atMost(1, Lits), fail.
eachDayMaxOneCourse.

eachDayWeekMaxNumHoursPerWeek :- hour(H), day(D), findall(cdh(C,D,H), course(C), Lits), courseHours(C,Hs), exactly(Hs,Lits), fail.
eachDayWeekMaxNumHoursPerWeek.

eachCourseExactlyItsYear :- course(C), findall(cy(C,Y), courseYear(C,Y), Lits), writeClause(Lits), fail.
eachCourseExactlyItsYear.
