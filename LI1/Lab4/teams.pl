...
incompatibleWorkers(W1,W2):- workerScore(W1,S1), workerScore(W2,S2), S is S1+S2, maxScore(Max), S > Max, W1 \= W2.
complexWorkers(W1, W2):- workerScore(W1, S1), workerScore(W2, S2), S is S1+S2, complexThreshold(Max), Max =< S, W1 \= W2.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1.- Declare SAT variables to be used
% wt(W,T) means "worker W is in team T"
satVariable( wt(W,T) ):- worker(W), team(T).
satVariable( t(T) ):- team(T).
...
writeClauses(MaxComplexTeams):-
    ...
    eachTeamAtLeastMinSize,
    eachTeamAtMostMaxSize,
    incompatibleWorkersRestriction,
    complexTeamsRestriction,
    complexWorkersRestriction(MaxComplexTeams),
    true,!.
...
eachTeamAtLeastMinSize :- team(T), findall(wt(W, T), worker(W), Lits), minSize(N), atLeast(N, Lits), fail.
eachTeamAtLeastMinSize.

eachTeamAtMostMaxSize :- team(T), findall(wt(W, T), worker(W), Lits), maxSize(N), atMost(N, Lits), fail.
eachTeamAtMostMaxSize.

incompatibleWorkersRestriction :- team(T), worker(W1), worker(W2), incompatibleWorkers(W1, W2), append([-wt(W1, T)], [-wt(W2, T)], Lits), writeClause(Lits), fail.
incompatibleWorkersRestriction.

complexTeamsRestriction :- team(T), worker(W1), worker(W2), complexWorkers(W1, W2), writeClause([-wt(W1, T), -wt(W2, T), t(T)]), fail.
complexTeamsRestriction.

complexWorkersRestriction(MaxComplexTeams) :- findall(t(T), team(T), Lits), atMost(MaxComplexTeams, Lits), fail.
complexWorkersRestriction(_).
...
costOfThisSolution(M,Cost):- findall(T, member(t(T), M), L), sort(L,L1), length(L1,Cost), !.
