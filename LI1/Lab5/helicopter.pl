...
heli( [], AccumulatedDistance, [CurrentPoint|RouteSoFar], [] ):- helicopterBasePoint(BasePoint), distanceBetweenTwoPoints(CurrentPoint, BasePoint, D), AD is AccumulatedDistance + D, storeRouteIfBetter(AD, [CurrentPoint|RouteSoFar]), fail.

% Pick up another passenger:
heli( PassengersToPickUp, AccumulatedDistance, [CurrentPoint|RouteSoFar], DestinationsOfPassengersInHelicopter ):- length(DestinationsOfPassengersInHelicopter, L), L < 2, select(Passenger, PassengersToPickUp, RestOfPassengers), getPassengerOrigen(Passenger, Origin), distanceBetweenTwoPoints(Origin, CurrentPoint, D), Distance is AccumulatedDistance + D, getPassengerDestination(Passenger, Destination), heli(RestOfPassengers, Distance, [Origin,CurrentPoint|RouteSoFar], [Destination|DestinationsOfPassengersInHelicopter]).

% Drop off one of the passengers in the helicopter:
heli( PassengersToPickUp, AccumulatedDistance, [CurrentPoint|RouteSoFar], DestinationsOfPassengersInHelicopter ):- select(Destination, DestinationsOfPassengersInHelicopter, RestOfDestinations), distanceBetweenTwoPoints(Destination, CurrentPoint, D), Distance is AccumulatedDistance + D, heli(PassengersToPickUp, Distance, [Destination,CurrentPoint|RouteSoFar], RestOfDestinations).

getPassengerOrigen([X,_], X).

getPassengerDestination([_,Y], Y).
