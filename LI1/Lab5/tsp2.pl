...
tsp( Cities, AccumulatedKm, _ ):- bestRouteSoFar(Km,_), lowerBound(Cities, LB), AccumulatedKm + LB >= Km, !, fail.
...
tsp( Cities, AccumulatedKm, [ CurrentCity | RouteSoFar ] ):-
    ...
    choose(City, Cities, RemainingCities, CurrentCity),
...
choose(City, Cities, RemainingCities, CurrentCity) :- getDistances(CurrentCity, Cities, Distances), sort(Distances, D), member(I, D), distance(CurrentCity, City, I), select(City, Cities, RemainingCities).
...
min([Min], Min) :- !.
min([X|L], Min) :- min(L, Min0), Min0 < X -> Min is Min0 ; Min is X.

lowerBound([], 0) :- !.
lowerBound([City|Cities], LB) :- lowerBound(Cities, LB0), mindist(City, D), LB is LB0 + D.
