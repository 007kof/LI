%REMPLAZAR EL SELECT POR MI_SELECT QUE DEVUELVA LAS CIUDADES ORDENADAS SEGÚN DISTANCIA
%Acum + Lowerbound > km, el lowerdbound es calcula amb la aritsta más corta de cada 
% remaining cities
%22 ciudades 
...
main:- bestRouteSoFar(Km,ReverseRoute), reverse( ReverseRoute, Route ),
...
tsp(Cities, AccumulatedKm, _ ):- bestRouteSoFar(Km,_), lowerbound(Cities,90000,Lowerbound), Heuristic is AccumulatedKm + Lowerbound, Heuristic  >= Km, !, fail.
tsp( Cities, AccumulatedKm, [ CurrentCity | RouteSoFar ] ):-
    mi_select1( City, Cities, RemainingCities ), % select next city to visit
...
%It get all the distances from a City to all the other Remainig Cities
getDistances(_,[],[]).
getDistances(City, [C|Cities],[D|Distances]):- distance(City, C, D),  getDistances(City,Cities,Distances). 

%It returns the minimum of a list
list_min([L|Ls], Min) :-
    list_min(Ls, L, Min).
...
%It gets the minimum distance over all the remaining cities
lowerbound([],a,a).
lowerbound([X|Cities],ResultAux, ResultAux):- row(X,Row), list_min(Row,Minimum), Result2 is min(ResultAux,Minimum), lowerbound(Cities,Result2, Result2).  
...
%mi_select(City,Cities, [NearestCity|L]):-
   %write('Mis cities:'), write(Cities),nl,
    %write('Mi city:'), write(City), nl,
    %getDistances(City,Cities,Distances),
    %write(Distances),
...
mi_select(City,Cities, [NearestCity|L]):-
    getDistances(City,Cities,Distances),
    getNearest(Cities, Distances, NearestCity),
    select(NearestCity,Cities,YOLO),  mi_select(City,YOLO,L).  

%  Number of points N is 10
distance(A,B,Km):-
...
row(A,Row):-
	...
	[ 154,205, 61, 51,123,227, 19,136,189, 99, 19,137,172,214,166,206,183,190,154, 85,  0, 98],
	[  55,153, 66, 49, 24,146, 79,113,111, 48,102, 39, 95,169,124,183,151,100, 91, 20, 98,  0]],
    nth1(A,M,RowAux), select(0,RowAux,Row), !.
