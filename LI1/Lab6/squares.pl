...
main:- 
    ...
    labeling([ff],L),
    displaySol(N,Sides,RowVars,ColVars), fail.

insideBigSquare(0, _, [], []) :- !.
insideBigSquare(N, B, [S|SL], [R|RL]) :- Max is B - S + 1, R in 1..Max, N1 is N - 1, insideBigSquare(N1, B, SL, RL).

nonoverlapping(_, [], [], []) :- !.
nonoverlapping(_, [S|SL], [R|RL], [C|CL]) :- nonoverlappingAux(S, SL, R, RL, C, CL), nonoverlapping(_, SL, RL, CL).

nonoverlappingAux(_,[],_,[],_,[]) :- !.
nonoverlappingAux(S, [S1|SL], R, [R1|RL], C, [C1|CL]) :- nonoverlappingAux(S, SL, R, RL, C, CL), correct(S, S1, R, R1, C, C1).

correct(S, S1, R, R1, C, C1) :- R + S #=< R1 ; R1 + S1 #=< R ; C + S #=< C1 ; C1 + S1 #=< C.
