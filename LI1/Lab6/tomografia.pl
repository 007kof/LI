...
p:-	ejemplo1(RowSums,ColSums),
	...
	declareConstraints(M, ColSums),
	label(L),
	pretty_print(RowSums,ColSums,MatrixByRows), nl.

listVars(0, []) :- !.
listVars(NVars, [_|L]) :- N is NVars - 1, listVars(N, L).

matrixByRows([], _, []) :- !.
matrixByRows(L, N, [Rows|M]) :- takeFirstNElements(L, N, L1), Rows = L1, append(L1, L2, L), matrixByRows(L2, N, M).

takeFirstNElements(L, N, R) :- append(R, _, L), length(R, N), !.

declareConstraints([], []) :- !.
declareConstraints([R|M], [X|L]) :- declareConstraints(M, L), sum(R, (#=), X).
