...
writeClauses(MaxNumYears):-
    eachProfessorCantHave2ClassAtTheSameTime,
    eachRoomOneCourse,
    restriccioclasses,
    ...
    eachcourseoneroom,
    maxCourseMorning(MaxNumYears),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

maxCourseMorning(N) :-
    course(C),
    findall(cdh(C,_,H), morning(H), L),
	atMost(N,L),
	fail.
maxCourseMorning(_).
...
costOfThisSolution(M,Cost):- findall(C, member(cdh(C,_,_), M), L), sort(L, L1), length(L1,Cost), !.
