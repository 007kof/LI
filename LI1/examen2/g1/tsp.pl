...
% For N=12 it should write: Optimal route: [1,5,4,11,7,3,10,8,2,6,9,1]. 612 km.
% No need to optimize your code.


main:- N=11, retractall(bestRouteSoFar(_,_)),  assertz(bestRouteSoFar(100000,[])),  % "infinite" distance
       findall(I,between(2,N,I),Cities), tsp( Cities, 0, [1], 1 ).
...
tsp( [], AccumulatedKm, [CurrentCity | RouteSoFar], ReturnCity ):- distance(CurrentCity, ReturnCity, D), AccumulatedKm1 is AccumulatedKm + D, storeRouteIfBetter(AccumulatedKm1,[ReturnCity, CurrentCity | RouteSoFar]), fail.
tsp(  _, AccumulatedKm, _          , _):- bestRouteSoFar(Km,_), AccumulatedKm >= Km, !, fail.
tsp( Cities, AccumulatedKm, [ CurrentCity | RouteSoFar ] , _):-
    ...
    tsp( RemainingCities, AccumulatedKm1, [ City, CurrentCity | RouteSoFar ] , _).
