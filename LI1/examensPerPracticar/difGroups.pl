...
incompatible(S1,S2):- %% returns a pair of students that coincided last year
    student(S1), student(S2), S1 < S2, S1 \= S2, lastYearTeam(T), member(S1, T), member(S2, T), true. % COMPLETE THIS!

%%%%%%  SAT Variables:
satVariable( st(S,T) ):- student(S), team(T). % student S goes to team T
satVariable( sst(S1, S2, T)) :- student(S1), student(S2), team(T).

writeClauses:- 
    eachTeamExactlyTeamSize,
    eachStudentExactly1Team,
    eachStudentAtMost1Incompatible,
    unir,
...
eachTeamExactlyTeamSize :- team(T), findall(st(S, T), student(S), Lits), teamSize(N), exactly(N, Lits), fail.
eachTeamExactlyTeamSize.

eachStudentExactly1Team :- student(S), findall(st(S, T), team(T), Lits), exactly(1, Lits), fail.
eachStudentExactly1Team.

eachStudentAtMost1Incompatible :- team(T), findall(sst(S1, S2, T), incompatible(S1, S2), Lits), atMost(1, Lits), fail.
eachStudentAtMost1Incompatible.

unir :- team(T), student(S1), student(S2), S1 < S2, S1 \= S2, writeClause([-st(S1, T), -st(S2, T), sst(S1, S2, T)]), fail.
unir.
