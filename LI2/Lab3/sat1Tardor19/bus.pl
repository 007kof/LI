...
satVariable( btd(B,C1,C2,D1,D2) ) :- bus(B), trip(C1-C2), consecutiveDays(D1,D2).

writeClauses:- 
    everyDayExactlyOneBusFromOneCity,
    everyDayCityOneBus,
    atLeastOneBusPerTrip,
    everyTripExactlyOneBus,
    relation,
    relation2,
    distanceRestriction,
...
everyDayExactlyOneBusFromOneCity :- day(D), city(C), findall(bdc(B,D,C), bus(B), Lits), exactly(1,Lits), fail.
everyDayExactlyOneBusFromOneCity.

everyDayCityOneBus :- bus(B), day(D), findall(bdc(B,D,C), city(C), Lits), exactly(1,Lits), fail.
everyDayCityOneBus.

atLeastOneBusPerTrip :- bus(B), consecutiveDays(D1,D2), findall(btd(B,C1,C2,D1,D2), trip(C1-C2), Lits), atLeast(1,Lits), fail.
atLeastOneBusPerTrip.

everyTripExactlyOneBus :- trip(C1-C2), findall(btd(B,C1,C2,D1,D2),(consecutiveDays(D1,D2), bus(B)), Lits), atLeast(1,Lits), fail.
everyTripExactlyOneBus.

relation :- bus(B), trip(C1-C2), consecutiveDays(D1,D2), writeClause([-bdc(B,D1,C1), -bdc(B,D2,C2), btd(B,C1,C2,D1,D2)]), fail.
relation.

relation2 :- bus(B), trip(C1-C2), consecutiveDays(D1,D2), writeClause([-btd(B,C1,C2,D1,D2), bdc(B,D1,C1)]), writeClause([-btd(B,C1,C2,D1,D2), bdc(B,D2,C2)]), fail.
relation2.

distanceRestriction :- bus(B), trip(C1-C2), trip(C2-C3), consecutiveDays(D1,D2,D3), dist(C1,C2,Dist1), dist(C2,C3,Dist2), maxDist(N), Dist is Dist1 + Dist2, Dist > N, writeClause([-btd(B,C1,C2,D1,D2), -btd(B,C2,C3,D2,D3)]), fail.
distanceRestriction.
