...
writeClauses:-
    eventsDayRestriction,
    moderatorDayRestriction,
    eventModeratorRestriction,
    oneEventPerDay,
    relacion,
    %negateNoExistingLits1,
    %negateNoExistingLits2,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

eventsDayRestriction :- day(D), findall(ed(E,D), (eventDays(E,Ds), member(D,Ds)), Lits), maxEventsPerDay(N), atMost(N,Lits), fail.
eventsDayRestriction.

moderatorDayRestriction :- moderator(M), findall(md(M,D), day(D), Lits), maxDaysPerModerator(N), atMost(N,Lits), fail.
moderatorDayRestriction.

eventModeratorRestriction :- eventModerators(E,Ms), findall(em(E,M),(moderator(M), member(M,Ms)), Lits), exactly(1,Lits), fail.
eventModeratorRestriction.

oneEventPerDay :- eventDays(E,Ds), findall(ed(E,D), (day(D), member(D,Ds)), Lits), exactly(1,Lits), fail.
oneEventPerDay.

relacion :- event(E), day(D), moderator(M), writeClause([-ed(E,D), -em(E,M), md(M,D)]), fail.
relacion.

negateNoExistingLits1 :- eventModerators(E,Ms), moderator(M), \+member(M,Ms), writeClause([-em(E,M)]), fail.
negateNoExistingLits1.

negateNoExistingLits2 :- eventDays(E,Ds), day(D), \+member(D,Ds), writeClause([-ed(E,D)]), fail.
negateNoExistingLits2.
