...
writeClauses(MaxNumProf):-
    eachCourseAndDayAtMostOneHour,
    notOverlapCoursesAndProfessors,
    eachCourseExactly1Professor,
    eachCourseExactly1Room,
    relationProfessor,
    relationRoom,
    notOverlapYear,
    relationYear,
    eachWeekExactlyNHours,
    maxNumProfessors(MaxNumProf),
    relationMaxProfessors,
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachCourseAndDayAtMostOneHour :- course(C), day(D), findall(cdh(C,D,H), hour(H), Lits), atMost(1, Lits), fail.
eachCourseAndDayAtMostOneHour.

notOverlapCoursesAndProfessors :- course(C1), course(C2), C1 > C2, day(D), hour(H), room(R), professor(P), writeClause([-crdh(C1,R,D,H), -crdh(C2,R,D,H)]), writeClause([-cpdh(C1,P,D,H), -cpdh(C2,P,D,H)]), fail.
notOverlapCoursesAndProfessors.

eachCourseExactly1Professor :- courseProfessors(C,LP), findall(cp(C,P), member(P,LP), Lits), exactly(1,Lits), fail.
eachCourseExactly1Professor.

eachCourseExactly1Room :- courseRooms(C,LR), findall(cr(C,R), member(R,LR), Lits), exactly(1,Lits), fail.
eachCourseExactly1Room.

relationProfessor :- course(C), professor(P), day(D), hour(H), writeClause([-cp(C,P), -cdh(C,D,H), cpdh(C,P,D,H)]), fail.
relationProfessor.

relationRoom :- course(C), room(R), day(D), hour(H), writeClause([-cr(C,R), -cdh(C,D,H), crdh(C,R,D,H)]), fail.
relationRoom.

notOverlapYear :- courseYear(C1,Y), courseYear(C2,Y), C1 > C2, day(D), hour(H), writeClause([-cdh(C1,D,H), -cdh(C2,D,H)]), fail.
notOverlapYear.

relationYear :- course(C), day(D), findall(cdh(C,D,H), hour(H), Lits), expressOr(cd(C,D), Lits), fail.
relationYear.

eachWeekExactlyNHours :- courseHours(C,Hs), findall(cd(C,D), day(D), Lits), exactly(Hs,Lits), fail.
eachWeekExactlyNHours.

maxNumProfessors(MaxNumProf) :- findall(p(P), professor(P), Lits), atMost(MaxNumProf,Lits), fail.
maxNumProfessors(_).

relationMaxProfessors :- course(C), professor(P), writeClause([-cp(C,P), p(P)]), fail.
relationMaxProfessors.
