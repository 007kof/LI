...
complexTeam(W1,W2) :- workerScore(W1,S1), workerScore(W2,S2), W1 \= W2, S is S1+S2, complexThreshold(Min), S >= Min.
...
writeClauses(MaxComplexTeams):-
    ...
    complexTeamRestriction,
    atMostComplexTeam(MaxComplexTeams),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

teamSizeRestriction :- team(T), findall(wt(W,T), worker(W), Lits), maxSize(Max), minSize(Min), atLeast(Min,Lits), atMost(Max,Lits), fail.
teamSizeRestriction.

oneWorkerOneTeam :- worker(W), findall(wt(W,T), team(T), Lits), exactly(1,Lits), fail.
oneWorkerOneTeam.

incompatibleWorkersRestriction :- incompatibleWorkers(W1,W2), team(T), writeClause([-wt(W1,T),-wt(W2,T)]), fail.
incompatibleWorkersRestriction.

complexTeamRestriction :- complexTeam(W1,W2), team(T), writeClause([-wt(W1,T), -wt(W2,T), t(T)]), fail.
complexTeamRestriction.

atMostComplexTeam(MaxComplexTeams) :- findall(t(T), team(T), Lits), atMost(MaxComplexTeams,Lits), fail.
atMostComplexTeam(_).
