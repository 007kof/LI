...
p:-	ejemplo1(RowSums,ColSums),
	...
	suma(MatrixByRows,RowSums),
	suma(MatrixByCols,ColSums),
	label([NVars]),
...
matrixByRows([],_,[]).
matrixByRows(L,N,[Y|R]) :- takefirstN(N,L,Y), removefirstN(N,L,L1), matrixByRows(L1,N,R).

takefirstN(0,_,[]).
takefirstN(N,[X|L],[X|R]) :- N1 is N - 1, takefirstN(N1,L,R).

removefirstN(0,L,L).
removefirstN(N,[_|L],R) :- N1 is N - 1, removefirstN(N1,L,R).

suma([],[]).
suma([X|L],[S|R]) :- suma(L,R), sum(X,#=,S).
