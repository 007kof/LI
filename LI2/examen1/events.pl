...
eachDayAtMostKEvents:-
    maxEventsPerDay(Max), day(D), findall(ed(E,D), event(E), Lits), atMost(Max,Lits), fail.
eachDayAtMostKEvents.
...
eachEventAtLeastKDays :- minEventsPerDay(Min), day(D), findall(ed(E,D), event(E), Lits), atLeast(Min,Lits), fail.
eachEventAtLeastKDays.

eventOrderRestriction :- day(D1), day(D2), dayForEvent(E1,D1), dayForEvent(E2,D2), E1 < E2, D1 > D2, writeClause([-ed(E1,D1), -ed(E2,D2)]), fail.
eventOrderRestriction.
